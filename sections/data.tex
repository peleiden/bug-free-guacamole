...
For the purposes of this model, a page is seen as a long sequence of words with hyperlinks spanning some of these.
...
The first and last tokens are always \texttt{[SEP]} and \texttt{[CLS]}.
%TODO: Check
...
Word spans over the tokens in each sequence are also include.
The set of all such sequences over all pages makes up the pretraining dataset.

\subsection{Entity Augmentation}
While the hyperlinks on Wikipedia provide useful entity span annotations for pretraining, they have one major drawback: A page is usually only linked once, even if it is referenced multiple times in an article.
This results in a large number of false negatives in the entity annotations.
\cite{yamada2020luke} do not account for this in their pretraining, however they use a simple string matching method for extending annotations when finetuning on the SQuAD dataset.

We preprocess the articles in a similar way already before the pretraining step to augment the entity part of the dataset.
Our preprocessing consists of the following tqo steps applied article-wise:
\begin{enumerate}
    ...
    \item For each title, check if it matches a sequence of characters following whitespace, ignoring casing.
    If there is a match, annotate and go to next whitespace.
\end{enumerate}
Many hyperlinks span only the first part of a given word.
This allows e.g. the words "Wind mills" to link to the article "Wind mill", even if they do not match exactly.
We allow such matches when annotating as we expect it to remove a large number of false negatives, while introducing relatively few false positives.
To reduce the number of false positives, we only perform extra annotations with titles of at least three characters.
This prevents the article on "A" to match every word starting with "A".
...
These steps increase the number of entity annotations from 4,693,505 to 6,614,552.

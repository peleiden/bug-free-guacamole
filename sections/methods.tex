...
\label{sub:dalpre}
\begin{enumerate}
    \item Find antal opmærksomhedsblokke.
    \item Fiks dimensionalitet af entity embeddings (og dobbeltjek ord)
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Entity-aware self-attention was used in the pretraining, as this was expected to positively impact the modeling of knowledge. This speculation is tested in Section~\ref{subsec:selfatt}.
    \item 
        ...
        Because of this, token embedding and contextualized representations reside in a 768 dimensional latent space rather than the 1024 dimensions used by LUKE \cite[Sec. 3.4]{yamada2020luke}.
    \item
...
When a better score than the hitherto best checkpoint was found, the checkpoint is overwritten.
...
\end{enumerate}
For each combination, the resulting model was evaluated on the development set, shown in Table~\ref{tab:hyperres} and the hyperparameters with highest average F1 score was chosen and the final model using which the final model was produced by fine-tuning again to avoid selection bias.
...
For instance, we discovered a bug in the masking where the unmasking and random word replacements would either be done or not done for an entire sequence, rather than being probabilistic for every masked subword token.

...
\begin{itemize}
    ...
    An example is predicting "Rings" in "Lord of the [MASK]", where predicting the single word is clearly easier than predicting the entire entity.
\end{itemize}
...
\label{sub:diffapp}
DaLUKE's pretraining largely follows LUKE's: Weights are initialized from da-BERT, and the same pretraining task is used on a corresponding dataset in Danish.

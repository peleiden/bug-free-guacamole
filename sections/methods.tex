...
Using the terminology of Table~\ref{tab:eval}, these measures are below defined for a class (such as LOC) below when assuming the predictions and ground truth both contain at least one positive label.
\begin{equation}
    \label{eq:precrec}
...
We, however, trained all models with the MISC annotation and also include it in our evaluations, as we are less focused on the practical use of the measure and more of its connection to language understanding.
...
\footnotetext{
    Here, the number of entire entity spans in each class is used, not the number of words annotated with the class; a unit used for other analysis in the discussion.
    The latter would overstate the amount of independent observations as two words in the same entity obviously share much information.
}
...
\label{sec:exidan}
A number of publicly available Danish NER models usable by NLP practitioners were collected and evaluated on the testing datasets of the three Danish NER annotations, considered in Section \ref{subsec:daNERdata}:
...
This effectively scales the loss and has no effect on the learning if the learning rate is adjusted accordingly.
...
\end{enumerate}
For each combination, the resulting model was evaluated on the development set, shown in Table~\ref{tab:hyperres}, and the hyperparameters with the highest average F1 score was chosen.
...
For instructions, see the read-me file on the repository.
...
However, we quickly ran into a number of issues with this approach, as getting all dependencies installed with the correct versions on our computing cluster proved to be a very challenging task.
...
Our code is inspired from the original LUKE code, but we have made efforts to make it more approachable focusing on code readability.
...
These ranged from domain specific such as accidentally leaving category pages in the entity vocabulary to general ones such as copying pointers instead of the memory stored at them.

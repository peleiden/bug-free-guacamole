...
When comparisons are made between model average F1 scores, an approximate 95\pro\ confidence interval is included calculated using the normality approximation for binomial observations \cite[Method 7.3]{brockhoff2018stat}.
That is, when denoting the average F1 score $f$ and the $n$ sample estimate thereof $\hat f$,
\begin{equation}
    \label{eq:ci}
    I = \hat f \pm z_{0.975}
    \sqrt{
...
\footnotetext{
    Here, the number of entire entity spans in each class is used, not the number of words annotated with the class, as is reported elsewhere.

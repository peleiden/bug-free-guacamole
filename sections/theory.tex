...
\end{itemize}
The text sequence is tokenized into $m$ subword tokens using the RoBERTa tokenizer.
The entities are all converted to $n$ entity tokens; integer values corresponding to indeces in a vocabulary of known, named entities.
The positions of the entities are included in the further inference as \emph{position ids}.

\paragraph{Entity Embeddings}
The subword tokens are forward passed through the same word embedding architecture as that of RoBERTa,

% The only architectural difference between LUKE and BERT in the \emph{pretraining} performed Yamada et al. (apart from pretraining specific heads) is the addition of a map from entity id's and entity position id's to entity embeddings.

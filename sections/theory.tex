...
Instead of sequentially letting each token update the state, the relevance of all tokens to all other tokens are calculated simultaneously, which can be done in parallel while not loosing any information to an iterative state update.
...
Motivated by the hope of this elusive generalizability, a number of methods have been proposed to enhance specifically the \emph{pretraining} of language models to better have handle factual knowledge, without using explicit knowledge graphs in the inference.
...
But the LUKE encoder does not only produce contextualized word representations (CWR); 
...
\begin{itemize}
    ...
    \item While transformers are good at capturing relationships between single words, they have difficulty modeling such relationships between sequences of words, of which entities often consist.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        $w_i, e_i, x_i$ all reside $\in \RR^H$ with $H=1024$ in the LUKE implementation.
    }
    \label{fig:luke}
\end{figure}\noindent

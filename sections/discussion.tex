...
Some further improvement which have the potential to push this method are proposed.
...
An immediate idea is to include the Norweigan Wikipedia as the language (At least \emph{bokmål}) is syntactically, and vocabulary wise similar to Danish.
...
A more comprehensive improvement could be extracted from our dataset augmentation idea, though it did not improve results at for the Danish Wikipedia.
...
It might hold that the quality of the entity annotations is not paramount, in which case tools that link sentences or paragraphs to entities without localizing them exactly might be used, as the entity potition ids thus could be omitted.
Here, Danish tools for explicit semantic analysis \cite{hansen2017esa} could help.

NÆVN GIGA-ORD!!!
\paragraph{Modelling}
Motivated by the impressive success, of DaCy, the most direct model improvement is to use the multilingual RoBERTa large

\begin{enumerate}
    \item Brug Multilingual RoBERTa Large
    \item Gentænke fremgangsmåde: Måske mere eksplicit vidensmodellering -- hvad med relationelle tokens?
\end{enumerate}

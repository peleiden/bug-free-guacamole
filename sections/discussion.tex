...
\end{enumerate}
Analysing when the model is right or wrong may provide valuable insight into how performance can be improved.
For this reason, a confusion matrix (table \ref{tab:pred-true-confmat}) is constructed for the DaNE testset that shows predicted labels against the true labels.
...
\begin{enumerate}
    \item LOC is often predicted on ORG entities -- but not the other way around.
    \item O is often predicted on true labels, especially on ORG and MISC. This highlights the relatively low recall (see table \ref{tab:DaNE}).
\end{enumerate}
Point 1 may have a relatively simple explanation: That many organizations have locations in their name, but not vice versa.
Consider for instance the organization entity "Karlslunde Hallerne".
While Karlslunde is a city and thus normally would be a LOC entity, that is not the case here.
As the model greedily selects from overlapping spans, LOC will be selected if the model is even slightly more sure that "Karlslunde" is a LOC entity than that "Karlslunde Hallerne" is an ORG entity.
Especially given the lower-case tokenization of the da-BERT tokenizer \cite{botxo2019dabert}, the capitalization of most organizations cannot be used, making predicting ORG's containing LOC's relatively harder.
This highlights a weakness of the greedy selection of partially overlapping spans.

To check if this is indeed the case, some examples of LOC predictions on ORG entities are printed out:
%TODO Er entitetsspannet indeholdende "Karlslunde" også ORG-markeret? Ellers skal nogle af pointerne ændres
%TODO Eksempler til punkt 1
%TODO Uddyb punkt 2

...
Even with this limit and with hyperparameters chosen to favour global structure, only messy, hairy ball plots, see Appendix~\ref{sec:dimredu}, were achieved from $t$-SNE and UMAP, motivating visualizing the data set where only the 4003 entity spans that corresponded to true entity annotations were included.
...
\begin{figure}[H]
    ...
    \caption{
        The first four components visualized against each other from PCA performed on the 975,312 possible entity spans in the DaNE training data.
    }
    \label{fig:all-pca}
\end{figure}\noindent
